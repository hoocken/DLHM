...
class VoxelizeCollateDemo:
    ...
    def __call__(self, batch):
        ...
        if ("train" in self.mode) and self.very_small_crops:
            batch = make_crops(batch)
        return voxelize_demo(batch, self.ignore_label, self.voxel_size, self.probing, self.mode,
                        task=self.task, ignore_class_threshold=self.ignore_class_threshold,
                        filter_out_classes=self.filter_out_classes, label_offset=self.label_offset,
                        num_queries=self.num_queries)


def voxelize_demo(batch, ignore_label, voxel_size, probing, mode, task,
    ...
    (coordinates, features, labels, original_labels, inverse_maps, original_colors, original_normals,
     original_coordinates, idx, clip_center_point, clip_feature) = (
        [],
        [],
        [],
        [],
    ...
    if "labels" in input_dict:
        ...
        if len(list_labels[0].shape) == 1:
            ...
            if clip_feature[0] is not None:
                ...
                sampled_idx = [np.random.choice(clip_feat.shape[0],
                                                num_queries,
                                                replace=clip_feat.shape[0] < num_queries)
                               for clip_feat in clip_feature]  # with replacement in case not enough clip features

                sampled_clip_feats = np.stack([clip_feat[sampled_id, :]
                                               for (clip_feat, sampled_id) in zip(clip_feature, sampled_idx)], axis=0)
                sampled_clip_pos = np.stack([clip_pos[sampled_id, :]
                                             for (clip_pos, sampled_id) in zip(clip_center_point, sampled_idx)], axis=0)
    ...
    if "train" not in mode:
        return (
            ...
            [sample[3] for sample in batch], sampled_clip_feats, sampled_clip_pos
        )
    else:
        return (
...
def get_instance_masks(list_labels, task, list_segments=None, ignore_class_threshold=100,
    ...
    for batch_id in range(len(list_labels)):
        ...
        full_ids = torch.div(full_ids, 1000, rounding_mode='trunc') * 1000 + \
                   np.unique(full_ids % 1000, return_inverse=True)[1] + 1
